...
assert.equal(report.visitedContents, 1); assert.equal(report.totalContents, 40); assert.equal(report.totalProblems, 120); assert.equal(report.review, 1); assert.equal(report.mastered, 1); assert.equal(report.practicing, 1); assert.equal(report.recentActivity.length, 2); assert.equal(report.units.some((unit) => unit.unit === problems[0].unit), true); assert.equal(report.contents[0].visited, true);
