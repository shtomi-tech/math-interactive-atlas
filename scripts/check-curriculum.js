...
assert.equal(order.length, contents.length); assert.equal(new Set(order).size, order.length); assert.deepEqual([...ids].sort(), [...order].sort());
...
console.log(`Curriculum: PASS (${contents.length} contents, 4 subjects, labels and all Practice references known)`);
