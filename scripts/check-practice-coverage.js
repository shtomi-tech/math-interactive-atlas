...
assert.equal(problems.length, 180, "Practice must contain 180 problems");
assert.equal(contents.length, 60, "Atlas must contain 60 contents");
assert.equal(new Set(problems.map((problem) => problem.id)).size, 180, "problem ids must be unique");
assert.equal(new Set(problems.map((problem) => problem.prompt)).size, 180, "problem prompts must be unique");
assert.equal(new Set(problems.map((problem) => problem.atlasContentId)).size, 60, "all Atlas contents must be covered");
...
console.log("Practice coverage: PASS (180 problems, 60/60 contents, difficulty 1/2/3)");
