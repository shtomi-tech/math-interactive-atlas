...
assert.equal(problems.length, contents.length * 3, "Practice must contain three problems per Atlas content");
assert.equal(new Set(problems.map((problem) => problem.id)).size, problems.length, "problem ids must be unique");
assert.equal(new Set(problems.map((problem) => problem.prompt)).size, problems.length, "problem prompts must be unique");
assert.equal(new Set(problems.map((problem) => problem.atlasContentId)).size, contents.length, "all Atlas contents must be covered");
...
console.log(`Practice coverage: PASS (${problems.length} problems, ${contents.length}/${contents.length} contents, difficulty 1/2/3)`);
