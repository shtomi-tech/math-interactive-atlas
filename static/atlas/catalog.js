import { SUBJECT_ORDER, SUBJECT_UNIT_ORDER, orderedContents, subjectLabel, unitLabel } from "./curriculum.js?v=20260913-r1";
...
function createCard(content, state, onSelect, onToggleFavorite, practiceProblems = [], repositoryAudits = new Map()) {
  ...
  const auditBadge = document.createElement("span"); auditBadge.className = `atlas-audit-status is-${audit.auditStatus || "pending"}`; auditBadge.textContent = auditLabel(audit.auditStatus); auditBadge.title = "外部Repositoryの監査状態"; footer.append(auditBadge);
...
export function renderCatalog(root, contents, { subject = null, unit = null, type = null, progress = null, query = "", state = {}, practiceProblems = [], repositoryAudits = new Map(), onSelect = () => {}, onFilterChange = () => {}, onToggleFavorite = () => {} }) {
