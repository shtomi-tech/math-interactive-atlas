import { SUBJECT_ORDER, SUBJECT_UNIT_ORDER, orderedContents } from "./curriculum.js?v=20260912-6g";
...
function searchMatch(content, query) { if (!query) return true; const haystack = [content.title, content.shortDescription, content.subjectLabel, content.unitLabel, content.formula, ...(content.discoveryPoints || [])].join(" ").toLocaleLowerCase(); return haystack.includes(query.toLocaleLowerCase()); }
...
function createCard(content, state, onSelect, onToggleFavorite) {
  ...
  const location = document.createElement("span"); location.className = "atlas-card-location"; location.textContent = `${content.subjectLabel}　＞　${content.unitLabel}`;
...
export function renderCatalog(root, contents, { subject = null, unit = null, type = null, progress = null, query = "", state = {}, onSelect = () => {}, onFilterChange = () => {}, onToggleFavorite = () => {} }) {
  ...
  appendOptions(subjectSelect, [["", "すべての科目"], ...SUBJECT_ORDER.filter((id) => contents.some((content) => content.subject === id)).map((id) => [id, contents.find((content) => content.subject === id).subjectLabel])]);
  ...
  function renderUnitOptions(preferred = unitSelect.value) { unitSelect.replaceChildren(); const available = availableUnits().filter((unitId) => contents.some((content) => content.unit === unitId)); appendOptions(unitSelect, [["", "すべての単元"], ...available.map((unitId) => { const content = contents.find((item) => item.unit === unitId); return [unitId, content.unitLabel]; })]); unitSelect.value = available.includes(preferred) ? preferred : ""; }
  ...
  function renderResults() {
    ...
    SUBJECT_ORDER.filter((subjectId) => visible.some((content) => content.subject === subjectId)).forEach((subjectId) => { const subjectContents = visible.filter((content) => content.subject === subjectId); const heading = document.createElement("h2"); heading.className = "atlas-catalog-subject"; heading.textContent = subjectContents[0].subjectLabel; results.append(heading); const unitOrder = SUBJECT_UNIT_ORDER[subjectId] || []; [...unitOrder, ...subjectContents.map((content) => content.unit).filter((value, index, values) => !unitOrder.includes(value) && values.indexOf(value) === index)].filter((unitId) => subjectContents.some((content) => content.unit === unitId)).forEach((unitId) => { const items = orderedContents(subjectContents.filter((content) => content.unit === unitId)); const section = document.createElement("section"); section.className = "atlas-catalog-unit"; const title = document.createElement("h3"); title.className = "atlas-catalog-unit-title"; title.textContent = items[0].unitLabel; const grid = document.createElement("div"); grid.className = "atlas-catalog-unit-grid"; items.forEach((content) => grid.append(createCard(content, normalizedState, onSelect, onToggleFavorite))); section.append(title, grid); results.append(section); }); });
  }
