...
function makeScene(context, config, spec) {
  ...
  const api = { state, setParameter(name,value){ if (!(name in state)) return; state[name]=spec.clamp?.(name,finite(value,state[name]),state) ?? finite(value,state[name]); spec.derive(state); if(!built){ spec.build(context,state,api,placers); built=true; } context.update(spec.summary(state),placers); }, reset(){Object.assign(state,initial); spec.derive(state); const centerLabels={centroid:"重心",circumcenter:"外心",incenter:"内心",orthocenter:"垂心",excenterA:"傍心"}; if(state.selected&&centerLabels[state.selected]) context.controls.querySelectorAll("button").forEach((button)=>button.setAttribute("aria-pressed",String(button.textContent===centerLabels[state.selected]))); context.update(spec.summary(state),placers);}, placers };
...
function centersScene(context,config){const labels={centroid:"重心",circumcenter:"外心",incenter:"内心",orthocenter:"垂心",excenterA:"傍心"};return makeScene(context,config,{initial:{cx:0,cy:2.5,selected:"centroid"},clamp:(name,value)=>name==="cx"?Math.max(-2.5,Math.min(2.5,value)):name==="cy"?Math.max(0.5,Math.min(4,value)):value,derive:s=>{s.A={x:-3,y:-2};s.B={x:3,y:-2};s.C={x:s.cx,y:s.cy};s.center=CENTER_FUNCS[s.selected]?.(s.A,s.B,s.C)??null;s.midAB={x:0,y:-2};s.midBC={x:(s.B.x+s.C.x)/2,y:(s.B.y+s.C.y)/2};s.midCA={x:(s.C.x+s.A.x)/2,y:(s.C.y+s.A.y)/2};s.bisector=angleBisectorFoot(s.A,s.B,s.C);},build:(c,s,api,p)=>{c.segment([()=>s.A.x,()=>s.A.y],[()=>s.B.x,()=>s.B.y]);c.segment([()=>s.B.x,()=>s.B.y],[()=>s.C.x,()=>s.C.y]);c.segment([()=>s.C.x,()=>s.C.y],[()=>s.A.x,()=>s.A.y]);c.point([()=>s.center?.x??0,()=>s.center?.y??0],{name:()=>labels[s.selected],fillColor:COLORS.highlight,strokeColor:COLORS.highlight,visible:()=>Boolean(s.center)});c.segment([()=>s.C.x,()=>s.C.y],[()=>s.midAB.x,()=>s.midAB.y],{strokeColor:COLORS.highlight,visible:()=>s.selected==="centroid"});c.segment([()=>s.A.x,()=>s.A.y],[()=>s.midBC.x,()=>s.midBC.y],{strokeColor:COLORS.highlight,visible:()=>s.selected==="centroid"});c.segment([()=>s.midAB.x,()=>s.midAB.y],[()=>s.midAB.x,()=>s.midAB.y+2],{strokeColor:COLORS.highlight,visible:()=>s.selected==="circumcenter"});c.segment([()=>s.A.x,()=>s.A.y],[()=>s.bisector?.x??0,()=>s.bisector?.y??0],{strokeColor:COLORS.highlight,visible:()=>s.selected==="incenter"||s.selected==="excenterA"});c.segment([()=>s.C.x,()=>s.C.y],[()=>s.C.x,()=>s.midAB.y],{strokeColor:COLORS.highlight,visible:()=>s.selected==="orthocenter"});const buttons={};Object.keys(labels).forEach(key=>buttons[key]=c.button(labels[key],key===s.selected,()=>{s.selected=key;Object.entries(buttons).forEach(([k,b])=>b.setAttribute("aria-pressed",String(k===key)));api.setParameter("cx",s.cx);}));p.push(c.touchTarget({label:"頂点C",position:()=>[s.C.x,s.C.y],onMove:(x,y)=>{api.setParameter("cx",Math.max(-2.5,Math.min(2.5,x)));api.setParameter("cy",Math.max(0.5,Math.min(4,y)));},onKey:e=>{if(e.key.startsWith("Arrow"))api.setParameter(e.key==="ArrowUp"||e.key==="ArrowDown"?"cy":"cx",(e.key==="ArrowLeft"||e.key==="ArrowDown"?-0.1:0.1)+(e.key==="ArrowUp"||e.key==="ArrowDown"?s.cy:s.cx));}}));},summary:s=>s.center?`${labels[s.selected]}：(${fixed(s.center.x)}, ${fixed(s.center.y)})`:`${labels[s.selected]}：この形では中心を定義できません`});}
